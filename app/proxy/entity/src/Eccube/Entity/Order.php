...
{
    use NameTrait, PointTrait, OrderTrait, \Customize\Entity\OrderTrait;
        ...
        foreach ($ProductOrderItems as $item_for_charge) {
            if ($item_for_charge->getProduct()->no_fee != 1) {
                ...
                if ($item_for_charge->getProduct()->shipping_charge == Null) {
                    ...
                    $shippingChargeSum += $item_for_charge->getProduct()->shipping_charge * (1+$item_for_charge->getTaxRate()/100) * $item_for_charge->getQuantity();
                }
            }
        }
        ...
        foreach ($this->getProductOrderItems() as $Item) {
            ...
            if ($Item->getProduct()->no_fee != 1) {
                ...
                if ($Item->getProduct()->shipping_charge == Null) {
                    ...
                    $cus_shipping_charge += $Item->getProduct()->shipping_charge * (1+$Item->getTaxRate()/100) * $Item->getQuantity();
                }
            }
            if ($flag == 0) {
