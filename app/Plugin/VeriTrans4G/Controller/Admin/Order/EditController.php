    ...
    private function modifyPayment(Request $request, $orderId, $mode)
    {
        ...
        try {
            ...
            if( $order->getPaymentMethod() == "銀行振込"){
                ...
                $mode = ($mode=="CAPTURE")?2:3;
              
                
                if($this->util->getOrderPayment($orderId)){
                    $Vt4gOrderPayment = $this->util->getOrderPayment($orderId);
                    $Vt4gOrderPayment->setMemo03(9999);
                    $Vt4gOrderPayment->setMemo04($mode);
                    $this->em->persist($Vt4gOrderPayment);
                    $this->em->flush();
                }
                else{
                    $Vt4gOrderPayment = new Vt4gOrderPayment();
                    $Vt4gOrderPayment->setOrderId($orderId);
                    $Vt4gOrderPayment->setMemo03(9999);
                    $Vt4gOrderPayment->setMemo04($mode);
                    $this->em->persist($Vt4gOrderPayment);
                    $this->em->flush();
