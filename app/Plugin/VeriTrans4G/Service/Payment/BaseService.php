...
class BaseService
{
    ...
    protected function shouldSendMail($order)
    {
        ...
        $isAmazonpay = $paymentId == $this->vt4gConst['VT4G_PAYTYPEID_AMAZONPAY'];
    ...
    public function operateCancel($payload)
    {
        ...
        switch ($payId) {
            case $this->vt4gConst['VT4G_PAYTYPEID_CREDIT']: // クレジットカード決済
                ...
                break;
            case $this->vt4gConst['VT4G_PAYTYPEID_AMAZONPAY']: // Amazon pay
                if($payload['orderPayment']->getMemo04() == 1){
                    
                    $mdkRequest = new \AmazonpayCancelRequestDto();
                }elseif($payload['orderPayment']->getMemo04() == 2){
                    
                    $mdkRequest = new \AmazonpayRefundRequestDto();
    ...
    public function operateCapture($payload)
    {
        ...
        switch ($payId) {
            ...
            case $this->vt4gConst['VT4G_PAYTYPEID_PAYPAL']: // paypal
                ...
                break;
            case $this->vt4gConst['VT4G_PAYTYPEID_AMAZONPAY']: // paypal
