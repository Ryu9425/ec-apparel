...
class PaymentRecvService extends BaseService
{
    ...
    public function saveRecvData($arrRecord)
    {
        ...
        foreach ($arrRecord as $key => $record) {
            ...
            if (isset($record['txnType']) && $record['txnType'] == 'recurring' && isset($record['mstatus']) && $record['mstatus'] == 'failure') {
                $vt4g_acountId = $record['accountId'];
                $d_str = $record['txnTime'];
                $year = substr($d_str, 0, 4);
                $month = substr($d_str, 4, 2);
                $day = substr($d_str, 6, 2);
                $h = substr($d_str, 8, 2);
                $m = substr($d_str, 10, 2);
                $s = substr($d_str, 12, 2);
                $r_d_str = $year . "/" . $month . "/" . $day . " " . $h . ":" . $m . ":" . $s;
                $lastTimeRecurringDateTime = new \DateTime($r_d_str);
                $lastTimeRecurringDateTime->modify('-1 month')->modify('last day of this month');
                $accountId = (int)$vt4g_acountId;
                $Customer = $this->em->getRepository(Customer::class)->find($accountId);
                $Customer->setCusCustomerLevel(1);
                $Customer->setVt4gAccountId("");
                $Customer->setLastTimeRecurringStatus(0);
                $Customer->setLastTimeRecurringDatetime($lastTimeRecurringDateTime);
                $this->em->flush();
            }
        }
    }
    ...
    private function checkAuthFailure($payTypeId, $record)
    {
        ...
        switch ($payTypeId) {
            case $this->vt4gConst['VT4G_PAYTYPEID_CREDIT']:
                ...
                if (
                    $record['mpiMstatus']  != $this->vt4gConst['VT4G_RESPONSE']['MSTATUS']['OK']
            ...
            case $this->vt4gConst['VT4G_PAYTYPEID_ALIPAY']:
                ...
                if (
                    $record['txnType'] == $this->vt4gConst['VT4G_RECV_TXN_TYPE_AUTH']
            ...
            case $this->vt4gConst['VT4G_PAYTYPEID_PAYPAL']:
                ...
                if (
                    $record['txnType'] == $this->vt4gConst['VT4G_RECV_TXN_TYPE_CAPTURE']
    ...
    private function execCreditRecvProcess($order, $orderPayment, $record, $payTypeId)
    {
        ...
        if (
            $this->pluginSetting['order_mail_timing_flg'] == $this->vt4gConst['VT4G_MAIL_TIMING']['ORDER']['ON_PAYMENT']
    ...
    private function execUPOPRecvProcess($order, $orderPayment, $record, $payTypeId)
    {
        ...
        if (!empty($memo09)) {
            ...
            foreach ($oldRecords as $oldRecord) {
                ...
                if (
                    $oldRecord['txnType']     == $record['txnType']
    ...
    private function execPayPalRecvProcess($order, $orderPayment, $record, $payTypeId)
    {
        ...
        if (!empty($memo09)) {
            ...
            foreach ($oldRecords as $oldRecord) {
                ...
                if (
                    $oldRecord['payerId']     == $record['payerId']
                ...
                if ($record['fixed'] == $this->vt4gConst['VT4G_RECV_PAYPAL_FIXED']) {
                    if (
                        ...
                        if (
                            $oldRecord['mstatus'] == $this->vt4gConst['VT4G_RESPONSE']['MSTATUS']['OK']
                            && $record['mstatus']    != $this->vt4gConst['VT4G_RESPONSE']['MSTATUS']['OK']
                        ) {
    ...
    private function checkPaymentStatusPushResult($orderPayment, $payStatuses, &$newPayStatus, &$newOrderStatus)
    {
        ...
        $msg = sprintf(
            trans('vt4g_plugin.payment.recv.payment_status.error'),
    ...
    private function checkOrderStatusPushResult($order, $orderStatus, &$newOrderStatus)
    {
        ...
        $msg = sprintf(
            trans('vt4g_plugin.payment.recv.order_status.error'),
...
__EOS__;
    ...
    private function makeRakutenReqMsg($record, $order_id = '', $recvType)
    {
        ...
        if (
            $recvType['VALUE'] == $this->vt4gConst['VT4G_PAY_STATUS']['REFUND']['VALUE']
            && $record['mstatus'] != $this->vt4gConst['VT4G_RESPONSE']['MSTATUS']['OK']
        ) {
