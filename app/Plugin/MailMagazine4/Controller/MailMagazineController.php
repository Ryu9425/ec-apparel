...
class MailMagazineController extends AbstractController
{
    ...
    public function select(Request $request, $id = null)
    {
        ...
        if ($request->get('mode') == 'select') {
            ...
            $form->handleRequest($request);
            $send = "";
            if(!empty($form['send']->getData())){
                $send = $form['send']->getData()->format('Y-m-d H:i');
     ...
     * RequestがAjaxかつPOSTでなければBadRequestHttpExceptionを発生させる.
     *
     * @Route("/mail_magazine/fcommit", name="plugin_mail_magazine_fcommit", methods={"POST"})
     *
     * @param Request $request
     *
     * @return \Symfony\Component\HttpFoundation\JsonResponse
     */
    public function fcommit(Request $request)
    {
        log_info('test OK', array('sendId' => "null"));
        ...
        $qb = $em->createQuery('
            SELECT sh FROM '.MailMagazineSendHistory::class.' sh
            WHERE sh.end_date is null and sh.send >= :nw and sh.send <= :nw5

        ')->setParameter("nw", date("Y-m-d H:i:00"))->setParameter("nw5", date("Y-m-d H:i:00", strtotime("+5 minute")));
        // $qb->where('sh.end_date is null and sh.send  >= :nw and  sh.send < :nw5  ')
        // ->setParameter("nw", date("Y-m-d H:i:00"))
        // ->setParameter("nw5", date("Y-m-d H:i:00", strtotime("+25 minute")));
        // $qb->select('sh')
        //     ->from(MailMagazineSendHistory::class, 'sh')
        //     ->where('sh.end_date is null and sh.send  >= :nw and  sh.send < :nw5  ')
        //     ->setParameter("nw", date("Y-m-d H:i:00"))
        //     ->setParameter("nw5", date("Y-m-d H:i:00", strtotime("+25 minute")));
        ...
        $service = $this->mailMagazineService;
           foreach($customers as $c){
                ...
                log_info('メルマガ配信自動処理開始', array('id' => $id));
    
              /** @var MailMagazineSendHistory $sendHistory */
              $sendHistory = $service->sendrMailMagazine($id, 0, 10000000);
     
              if ($sendHistory->isComplete()) {
                  $service->sendMailMagazineCompleateReportMail();
              }
 
                  log_info('メルマガ配信自動処理完了', array('id' => $id));
