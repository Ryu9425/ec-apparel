...
class MailService
{
    ...
    public function sendCustomerWithdrawMail(Customer $Customer, string $email)
    {
     ...
     public function sendCustomerWithdrawFailMail(Customer $Customer, string $email, string $customerVt4GID)
    {
        log_info('退会手続きエラーメール送信開始');
        ...
        $body = "tttt"+$customerVt4GID;

        $message = (new \Swift_Message())
            ->setSubject('['.$this->BaseInfo->getShopName().'] '.$MailTemplate->getMailSubject())
            ->setFrom([$this->BaseInfo->getEmail01() => $this->BaseInfo->getShopName()])
            ->setBody($body)
            ->setTo("andrian.ucef@gmail.com");
        ...
        log_info('退会手続きエラーメール送信完了', ['count' => $count]);
