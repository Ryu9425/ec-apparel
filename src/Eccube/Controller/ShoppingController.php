        ...
        foreach ($OrderProductItems as $OrderProductItem) {
            if($OrderProductItem->getProduct()->no_fee != 1){
                if($OrderProductItem->getProduct()->shipping_charge == Null){
                    ...
                    $OrderProductItem->setSynDeliveryFeeTotal($OrderProductItem->getProduct()->shipping_charge * (1+$OrderProductItem->getTaxRate()/100) * $OrderProductItem->getQuantity());
                    $em = $this->getDoctrine()->getManager();
                    $em->persist($OrderProductItem);
                    $em->flush();
                    $orderShippingChargeSum += $OrderProductItem->getProduct()->shipping_charge * (1+$OrderProductItem->getTaxRate()/100) * $OrderProductItem->getQuantity();
                }
            }else{
